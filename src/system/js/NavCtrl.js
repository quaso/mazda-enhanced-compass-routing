...
NavCtrl.prototype = {
    ...
    createAlertsLayer: function () {
        ...
        for (var i = 0; i < ALERTS.length; i++) {
            var feature = new ol.Feature(new ol.geom.Point(ol.proj.fromLonLat([ALERTS[i].lng, ALERTS[i].lat])));
    ...
    showMenu: function (open) {
        if (open) {
            this.selectAndShowActiveMenu("mainMenu");
    ...
    handleMenuEvent: function (event) {
        ...
        switch (event) {
            case "select":
                var closeAfterSelect = MenuManager.getInstance().executeMenuItemAction();
                var hide = false;
    ...
    setLocationData: function (location) {
        if (location.isValidInfo) {
            ...
            if (typeof (location.latlng) == "object") {
                var latLng = new LatLng(location.latlng.lat, location.latlng.lng);
                ...
                if (this.navigationMode) {
                    ...
                    if (route != null) {
                        if (typeof (route.text) != "undefined") {
                            if (SETTINGS.debug) {
                                this.showNotification(route.text, -1);
                            }
                        } else if (typeof (route.directions) != "undefined") {
    ...
    clearRoute: function (hideRouteDisplay) {
        ...
        this.offlineNavigationEnd();
        for (var i = 0; i < this.routeLayer.length; i++) {
    ...
    showAllRoutesToDestination: function (show, dest) {
        if (show) {
            OfflineNavigation.getInstance().findCachedRoutesToDestination(dest, function (routes) {
                for (var i = 0; i < routes.length; i++) {
                    // this.showRoute(routes[i].data.path, "blue");
                    __NavPOICtrl.showRoute(routes[i].full_path, "blue");
                }
                OfflineNavigation.getInstance().setPosition(
                    new LatLng(__NavPOICtrl.mapProps.currentLatitude, __NavPOICtrl.mapProps.currentLongitude));
            });
        } else {
            __NavPOICtrl.clearRoute(false);
        }
    },
    ...
    hideOffRouteDiv: function () {
        this.setHidden(this.offRouteDiv);
        for (var i = 0; i < this.offRouteImg.length; i++) {
