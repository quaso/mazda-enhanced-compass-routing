    ...
    function init() {
        ...
        function findNearestRoute(position) {
            ...
            for (var i = 0; i < this.routes.length; i++) {
                var nearestPoint = this.routes[i].nearestTo(position, 0, 0);
                if (nearestPoint.distance < result.distance) {
                    result = {
                        distance: nearestPoint.distance, route: this.routes[i], onRoute: false
                    };
                }
            }
        ...
        return {
            ...
            findCachedRoutesToDestination: function (dest, callback) {
                RoutesCache.getInstance().readMoreFromCache(dest, function (cachedRoutes) {
                    this.routes = [];
                    for (var i = 0; i < cachedRoutes.length; i++) {
                        var route = new Route().parse(cachedRoutes[i].data);
